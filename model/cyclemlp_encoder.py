...
class CycleNet(nn.Module):
    ...
    def __init__(self, layers, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        norm_layer=nn.LayerNorm, mlp_fn=CycleMLP, fork_feat=True):
...
@register_model
class CycleMLP_B4(CycleNet):
    def __init__(self, **kwargs):
        transitions = [True, True, True, True]
        layers = [3, 8, 27, 3]
        mlp_ratios = [8, 8, 4, 4]
        embed_dims = [64, 128, 320, 512]
        super(CycleMLP_B4, self).__init__(
            layers, embed_dims=embed_dims, patch_size=7, transitions=transitions,
            mlp_ratios=mlp_ratios, mlp_fn=CycleMLP, **kwargs)
